defmodule SpeakFirstAi.SubscriptionPlansTest do
  ...
  describe "subscription_plan" do
    ...
    @invalid_attrs %{active: nil, name: nil, description: nil, currency: nil, price_cents: nil, interval: nil, stripe_price_id: nil, trial_period_days: nil}
    ...
    test "get_subscription_plan!/2 returns the subscription_plan with given id" do
      ...
      subscription_plan = subscription_plan_fixture(scope)
      other_scope = user_scope_fixture()
      assert SubscriptionPlans.get_subscription_plan!(scope, subscription_plan.id) == subscription_plan
      assert_raise Ecto.NoResultsError, fn -> SubscriptionPlans.get_subscription_plan!(other_scope, subscription_plan.id) end
    end

    test "create_subscription_plan/2 with valid data creates a subscription_plan" do
      valid_attrs = %{active: true, name: "some name", description: "some description", currency: "some currency", price_cents: 120.5, interval: "some interval", stripe_price_id: "some stripe_price_id", trial_period_days: 42}
      scope = user_scope_fixture()

      assert {:ok, %SubscriptionPlan{} = subscription_plan} = SubscriptionPlans.create_subscription_plan(scope, valid_attrs)
    ...
    test "create_subscription_plan/2 with invalid data returns error changeset" do
      scope = user_scope_fixture()
      assert {:error, %Ecto.Changeset{}} = SubscriptionPlans.create_subscription_plan(scope, @invalid_attrs)
    end
    ...
    test "update_subscription_plan/3 with valid data updates the subscription_plan" do
      ...
      assert {:ok, %SubscriptionPlan{} = subscription_plan} = SubscriptionPlans.update_subscription_plan(scope, subscription_plan, update_attrs)
    ...
    test "update_subscription_plan/3 with invalid data returns error changeset" do
      scope = user_scope_fixture()
      subscription_plan = subscription_plan_fixture(scope)
      assert {:error, %Ecto.Changeset{}} = SubscriptionPlans.update_subscription_plan(scope, subscription_plan, @invalid_attrs)
      assert subscription_plan == SubscriptionPlans.get_subscription_plan!(scope, subscription_plan.id)
    end

    test "delete_subscription_plan/2 deletes the subscription_plan" do
      scope = user_scope_fixture()
      subscription_plan = subscription_plan_fixture(scope)
      assert {:ok, %SubscriptionPlan{}} = SubscriptionPlans.delete_subscription_plan(scope, subscription_plan)
      assert_raise Ecto.NoResultsError, fn -> SubscriptionPlans.get_subscription_plan!(scope, subscription_plan.id) end
    end
    ...
    test "delete_subscription_plan/2 with invalid scope raises" do
      ...
      other_scope = user_scope_fixture()
      subscription_plan = subscription_plan_fixture(scope)
      assert_raise MatchError, fn -> SubscriptionPlans.delete_subscription_plan(other_scope, subscription_plan) end
    end

    test "change_subscription_plan/2 returns a subscription_plan changeset" do
      scope = user_scope_fixture()
      subscription_plan = subscription_plan_fixture(scope)
      assert %Ecto.Changeset{} = SubscriptionPlans.change_subscription_plan(scope, subscription_plan)
    end
  end
end
