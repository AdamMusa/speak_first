defmodule SpeakFirstAi.ConversationTest do
  ...
  describe "conversations" do
    ...
    test "get_conversation_topic!/2 returns the conversation_topic with given id" do
      ...
      conversation_topic = conversation_topic_fixture(scope)
      other_scope = user_scope_fixture()
      assert Conversation.get_conversation_topic!(scope, conversation_topic.id) == conversation_topic
      assert_raise Ecto.NoResultsError, fn -> Conversation.get_conversation_topic!(other_scope, conversation_topic.id) end
    end
    ...
    test "create_conversation_topic/2 with valid data creates a conversation_topic" do
      ...
      assert {:ok, %ConversationTopic{} = conversation_topic} = Conversation.create_conversation_topic(scope, valid_attrs)
    ...
    test "create_conversation_topic/2 with invalid data returns error changeset" do
      scope = user_scope_fixture()
      assert {:error, %Ecto.Changeset{}} = Conversation.create_conversation_topic(scope, @invalid_attrs)
    end
    ...
    test "update_conversation_topic/3 with valid data updates the conversation_topic" do
      ...
      assert {:ok, %ConversationTopic{} = conversation_topic} = Conversation.update_conversation_topic(scope, conversation_topic, update_attrs)
    ...
    test "update_conversation_topic/3 with invalid data returns error changeset" do
      scope = user_scope_fixture()
      conversation_topic = conversation_topic_fixture(scope)
      assert {:error, %Ecto.Changeset{}} = Conversation.update_conversation_topic(scope, conversation_topic, @invalid_attrs)
      assert conversation_topic == Conversation.get_conversation_topic!(scope, conversation_topic.id)
    end

    test "delete_conversation_topic/2 deletes the conversation_topic" do
      scope = user_scope_fixture()
      conversation_topic = conversation_topic_fixture(scope)
      assert {:ok, %ConversationTopic{}} = Conversation.delete_conversation_topic(scope, conversation_topic)
      assert_raise Ecto.NoResultsError, fn -> Conversation.get_conversation_topic!(scope, conversation_topic.id) end
    end
    ...
    test "delete_conversation_topic/2 with invalid scope raises" do
      ...
      other_scope = user_scope_fixture()
      conversation_topic = conversation_topic_fixture(scope)
      assert_raise MatchError, fn -> Conversation.delete_conversation_topic(other_scope, conversation_topic) end
    end
